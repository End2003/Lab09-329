...
<template>
  ...
  <section class="vh-100" >
    <div><h1 style="text-align: center;" class="text_box"><strong>สภาพอากาศสถานที่ {{travelItem.name}} </strong></h1></div>
    <div class="container py-5 h-100">
      <div class="row d-flex justify-content-center align-items-center h-100">
        <div class="col-md-8 col-lg-6 col-xl-4" v-for="(i,index) in items" :key="index">
  
        <div class="card" style="color: #0051f2; border-radius: 35px;" >
            ...
            <div class="d-flex flex-column text-center mt-5 mb-4">
              ...
              <span class="small" style="color: #827fb6">{{ i.weather[0].main }}</span>
            </div>
            <div class="d-flex align-items-center">
              <div class="flex-grow-1" style="font-size: 1rem;">
                  <div><i class="fas fa-wind fa-fw" style="color: #000000;"></i> <span class="ms-1"> ความเร็วลม: {{ i.wind.speed }} ms/s </span></div>
                  <div><i class="fas fa-tint fa-fw" style="color: #000000;"></i> <span class="ms-1"> ระดับความชื้น: {{ i.main.humidity }}% </span></div>
                  <div><i class="fas fa-sun fa-fw" style="color: #000000;"></i> <span class="ms-1"> คำอธิบาย : {{ i.weather[0].description }} </span></div>
              </div>
...
.card {
  ...
  border: none;
  border-radius: 35px;
  box-shadow: 0px 15px 20px rgba(31, 0, 109, 0.603);
  overflow: hidden;
  position: relative;
  z-index: 1;
}
...
.card:hover {
  box-shadow: 0px 15px 20px rgba(169, 0, 231, 0.6);
}
.card{
  margin-bottom: 50px;
}
